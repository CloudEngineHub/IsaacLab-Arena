...
def _no_op_subtask_config(object_ref: str) -> SubTaskConfig:
    ...
    Used for both the ``body`` and ``left`` channels in ``StaticPickAndPlaceMimicEnvCfg``:
    the channel runs to the end of the demo (no ``subtask_term_signal``), Mimic plays
    back the recorded trajectory verbatim (``action_noise=0``, no interpolation), and
    the ``nearest_neighbor_object`` strategy is effectively a no-op because there is
    only one subtask to select from.
