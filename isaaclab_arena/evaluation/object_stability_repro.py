...
This is intentionally small and CLI-oriented: run once with
``--origin_main_solver`` to emulate the old solver behavior, then run the same
scene without that flag (and optionally with ``--force_convex_hull``) to verify
the fix.
...
Visual repro shape::
    ...
    /isaac-sim/python.sh isaaclab_arena/evaluation/object_stability_repro.py \
        ...
        --objects parmesan_cheese_canister_hope_robolab mustard_bottle_hope_robolab \
            bbq_sauce_bottle_hope_robolab ranch_dressing_hope_robolab
